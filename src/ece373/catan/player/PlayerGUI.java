...
public class PlayerGUI extends JPanel {
	...
	private class ButtonListener implements ActionListener
	{
		...
		public void handleRoadBuilding() {
			...
			int input = JOptionPane.showOptionDialog(null, "Press OK to build a road. Can be used twice.", "Road Building Card", JOptionPane.OK_CANCEL_OPTION, JOptionPane.INFORMATION_MESSAGE, null, null, null);
			...
			if(input == JOptionPane.OK_OPTION)
			{
				for(DevelopmentCard dc: player.getDevelopmentCards()) {
					if(dc instanceof RoadBuildingCard) {
						...
						if(((RoadBuildingCard) dc).getUsedOnce() == true) {
							player.removeRoadBuildingCard();
							break;
						}
						else {
							((RoadBuildingCard) dc).setUsedOnce(true);
							break;
						}
					}
				}
